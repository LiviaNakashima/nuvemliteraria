...
router.post('/entrar', function (req, res, next) {
  ...
  banco.conectar().then(() => {
    ...
    return banco.sql.query(`select * from DadoPessoal where Email='${login}' and Senha='${senha}'`);
  }).then(consulta => {
...
router.post('/cadastrar', function (req, res, next) {
  ...
  banco.conectar().then(() => {
	...
	nome = req.body.nomec; // depois de .body, use o nome (name) do campo em seu formulário de login
    ...
    return banco.sql.query(`select count(*) as contagem from DadoPessoal where email = '${login}'`);
  }).then(consulta => {
    ...
    res.status(500).send(erro);
	  ...
	  if (cadastro_valido) {		  
		...
		banco.sql.query(`insert into DadoPessoal (Nome, Email, Senha) values ('${nome}','${login}','${senha}')`).then(function() {
...
router.post('/registrar', function (req, res, next) {
  ...
  banco.conectar().then(() => {
    console.log(`Chegou p/ cadastro: ${JSON.stringify(req.body)}`);
	  livro = req.body.livro; // depois de .body, use o nome (name) do campo em seu formulário de login
	...
	if (consulta.recordset[0].contagem >= 1) {
		res.status(400).send(`Já existe o livro "${livro}"`);
		return;
    } else {
		console.log('Válido!');
		cadastro_valido = true;
